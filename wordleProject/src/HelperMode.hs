...
import ListUtils ( isValidIndex, indexToStartCountingFrom, emptyList, getIndexes, getLettersFromColor)
import GameState()
...
getWordsWithYellowLetters :: [[Char]] -> [(Int, Char, Color)] -> [(Int, [Char])]
getWordsWithYellowLetters initialDict colorMatchingList =
    let wordsWithGreenLetters = zip [1..] (getWordsMatchingGreenLetters initialDict colorMatchingList)
        ...
        indexesOfGreenLetters = getIndexes colorMatchingList
...
getValidWords initialDict colorMatchingList =
    let wordsWithYellowAndGreenLetters = getWordsWithYellowLetters initialDict colorMatchingList
