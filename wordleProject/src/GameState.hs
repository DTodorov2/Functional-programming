module GameState ( updateGameStateEasyMode, getFiltersCount, updateGameState, GameState(greenLetters, yellowLetters, grayLetters), getAllFilters, emptyGameState) where
...
-- getCountOfLetterInGameState :: Char -> [(Int, Char, Color)] -> Int
-- getCountOfLetterInGameState letter neededLetters =
--     length [x | (_, x, _) <- neededLetters, x == letter]
...
updateYellowLetters yellowLettersGameState (pair:restMapCountYellowLetter) newList =
    ...
    in  if countLetterInGameState < count
        then updateYellowLetters yellowLettersGameState restMapCountYellowLetter (newList ++ replicate count (-1, letter, Yellow))
        else updateYellowLetters yellowLettersGameState restMapCountYellowLetter (newList ++ replicate countLetterInGameState (-1, letter, Yellow))
