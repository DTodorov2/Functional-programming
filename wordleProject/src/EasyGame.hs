...
import GameState ( GameState (..), emptyGameState, updateGameStateEasyMode, getAllFilters )
...
confirmWordUsage guess gameState dict secretWord = do
    ...
    if notInDict
        then
            ...
            putStrLn ("iima li contradiction: " ++ show hasNoContradiction)
    ...
    in  if null actualGameStateYellowLetters 
            then return True
                ...
                then do
                    putStrLn errorMessage
                    return False
                else if null newYellowLetters || all (`elem` actualGameStateYellowLetters) newYellowLetters
                        then return True
                        else do
                            putStrLn errorMessage
                            return False
...
--         actualGameStateYellowLetters = removeLettersFromFirstList yellowLettersGameState newGreenLetters
--         errorMessage = redColor ++ "Отсъстват жълти букви, за които се знае, че присъстват в думата!" ++ whiteColor
--     in  if length newYellowLetters /= length actualGameStateYellowLetters
--             then do
--                 putStrLn errorMessage
--                 return False
--             else if (null newYellowLetters && null actualGameStateYellowLetters) ||
--                     (not (null newYellowLetters) && all (`elem` actualGameStateYellowLetters) newYellowLetters)
--                     then return True
--                     else do
--                         putStrLn errorMessage
--                         return False
